...
If you are upgrading from `v2.2.0`, set `UPGRADE_FROM_LAST_VERSION=1` to enable upgrading and configure the following:
...
Then, copy the OpenZeppelin artifacts for the official deployed `v2.2.0` contracts from `.openzeppelin/mainnet.json` to `.openzeppelin/unknown-1337.json`. If you decide to disable upgrading by setting restoring `UPGRADE_FROM_LAST_VERSION=0`, make sure to delete `.openzeppelin/unknown-1337.json`.
...
To run automated tests on the contracts on your private mainnet fork, run `npm test` (which runs `npm run ganache` in the background for you). If you are upgrading from `v2.2.0`, you must also set the following variables in `.env`:
    ...
    UPGRADE_FUND_PROXY_ADDRESS=0xB202cAd3965997f2F5E67B349B2C5df036b9792e
