...
* **Get my RSPT balance (internal representation of my USD balance supplied to fund):** `uint256 RariFundToken.balanceOf(address account)` returns the amount of RSPT owned by `account`.
...
* Your Rari Stable Pool Token balance is a *token-based representation of your fund balance.*
    * RSPT is minted to you when you deposit to the fund and redeemed (i.e., burned) when you withdraw from the fund.
    * Accruing interest increases your USD fund balance, meaning the USD value of your RSPT increases. However, your RSPT balance itself does not increase: instead, the exchange rate of RSPT increases at the same rate as every user's balance as they accrue interest.
    * When you transfer your RSPT, you transfer your holdings supplied to the fund (deposits + interest).
...
* **Get total USD supplied (by all users):** `uint256 RariFundManager.getFundBalance()` returns the fund's total investor balance (all RSPT holders' funds but not unclaimed fees) of all currencies in USD (scaled by 1e18).
* **Get total interest accrued (by all users):** `int256 RariFundManager.getInterestAccrued()` returns the total amount of interest accrued by past and current RSPT holders (excluding the fees paid on interest) in USD (scaled by 1e18).
